<#
.SYNOPSIS
  CP2077 Local AI Modding -- Easy Mode One-Click Installer (v4.3)
...
.DESCRIPTION
  ...
  v4.3 vs v4.2 (BUG-FIX RELEASE):
    - v4.2 pulled 'qwen3-coder:7b' (does not exist on Ollama -- no 7B
      variant is published; smallest qwen3-coder is 30B) and
      'qwen3-coder:30b-a3b' (needs an explicit quantization suffix).
      Both pulls failed with 'pull model manifest: file does not exist'
      leaving users with no coder model. v4.3 pulls tags that exist:
        * baseline: qwen2.5-coder:7b           (~4.7 GB, exists)
        * big:      qwen3-coder:30b-a3b-q4_K_M (~19 GB,  exists)
    - verify-install.ps1 now accepts EITHER qwen2.5-coder* or
      qwen3-coder* as a valid coder model (previously only looked for
      qwen3-coder*, falsely failing clean runs).
...
# 0. Welcome
# ---------------------------------------------------------------------------
Write-Banner "CP2077 Local AI -- Easy Mode Installer (v4.3)"
Write-Host ""
...
Write-Host "    4. Qwen2.5-Coder 7B  (the AI brain, baseline) ~4.7 GB"
Write-Host ""
...
# ---------------------------------------------------------------------------
# NOTE on tag choice (v4.3): earlier versions pulled 'qwen3-coder:7b' which
# DOES NOT EXIST on ollama.com/library/qwen3-coder -- there is no 7B
# variant published. The smallest qwen3-coder is the 30B model. So for the
# "baseline fits on an 8 GB GPU" slot we use qwen2.5-coder:7b, which IS
# published (4.7 GB) and is the most widely-used small code model.
$BASELINE_MODEL = 'qwen2.5-coder:7b'
$BIG_MODEL      = 'qwen3-coder:30b-a3b-q4_K_M'   # ~19 GB, the actual published tag

Write-Phase 4 "Download the AI brain ($BASELINE_MODEL, ~4.7 GB)"
...
if ($ollamaExe) {
    ...
    function Test-OllamaModel([string]$wanted) {
        # Exact match only for the versioned/quantized tag we asked for.
        # (Prefix match would treat e.g. 'qwen2.5-coder:14b' as satisfying
        # a request for 'qwen2.5-coder:7b', which is wrong.)
    ...
    if (Test-OllamaModel $BASELINE_MODEL) {
        Write-Skip "$BASELINE_MODEL already downloaded."
    } else {
        Write-Note "Pulling $BASELINE_MODEL -- the big download. Time depends on your connection."
        & $ollamaExe pull $BASELINE_MODEL
        if ($LASTEXITCODE -eq 0) { Write-Ok "$BASELINE_MODEL downloaded." } else { Write-Err "Pull failed (exit $LASTEXITCODE). Re-run this script to retry." }
    }

    # ---------------------------------------------------------------------------
    # 5. Optional bigger model
    # ---------------------------------------------------------------------------
    Write-Phase 5 "Optional: bigger / smarter model (Qwen3-Coder 30B-A3B, ~19 GB)"

    if (Test-OllamaModel $BIG_MODEL) {
        Write-Skip "$BIG_MODEL already downloaded."
    } elseif ($SkipOptionalModel) {
        Write-Skip "Skipping (SkipOptionalModel flag set)."
    } else {
        Write-Host ""
        Write-Host "  The 30B-A3B model is much smarter for hard tasks but is a 19 GB"
        ...
        if (Confirm-YN 'Download the bigger 30B-A3B model now?' 'N') {
            & $ollamaExe pull $BIG_MODEL
            if ($LASTEXITCODE -eq 0) { Write-Ok "$BIG_MODEL downloaded." } else { Write-Err "Pull failed (exit $LASTEXITCODE)." }
        } else {
...
Write-Host "       - Model:         pick  qwen2.5-coder:7b"
