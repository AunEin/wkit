<#
.SYNOPSIS
  CP2077 Local AI Modding Setup Checker v4.0 (Cline + VS Code Edition)

.DESCRIPTION
  Verifies the v4.0 "Easy Mode" stack:
    - NVIDIA GPU + driver
...
Write-Host "  CP2077 Local AI Modding Setup Checker v4.0" -ForegroundColor Magenta
...
if ($gpuName) {
    ...
    elseif ($vramMB -ge 12000) { Write-Warn "VRAM: 12-16 GB -- 30B-A3B will partially offload to CPU; smoother with qwen3-coder:7b" }
...
# v4.0 baseline: only qwen3-coder:7b is required, others are optional
if (Test-ModelInstalled 'qwen3-coder:7b') {
    Write-Ok ("Model: qwen3-coder:7b  --  Required baseline (~5 GB)")
} else {
    Write-Err ("Model missing: qwen3-coder:7b -- the baseline AI brain")
    Set-NextStep "Run INSTALL-EVERYTHING.bat (or:  ollama pull qwen3-coder:7b)"
}

foreach ($m in @(
    @{ name='qwen3-coder:30b-a3b'; label='Optional bigger model (16 GB VRAM, smarter)' },
