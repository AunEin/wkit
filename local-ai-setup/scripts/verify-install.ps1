<#
.SYNOPSIS
  CP2077 Local AI -- end-to-end smoke test (v4.3)
...
.DESCRIPTION
  ...
  Tests, in order:
    ...
    3. Ollama generates a token from a coder model (proves GPU loads)
  ...
  v4.3 fix: accepts EITHER qwen2.5-coder* OR qwen3-coder* as a valid
  "Cline-ready coder model". v4.1/4.2 only looked for qwen3-coder*,
  which falsely failed after a clean install because that family has
  no small (7B) tag on Ollama -- the baseline installer actually pulls
  qwen2.5-coder:7b.
...
if ($tags -and $tags.models) {
    # Preference order: exact baseline tag the installer pulls, then any
    # qwen2.5-coder, then any qwen3-coder, then any *-coder. Anything
    # matching counts as a working coder model for Cline's purposes.
    $names = @($tags.models | ForEach-Object { $_.name })
    foreach ($m in $tags.models) { Info ("found: {0}" -f $m.name) }

    $preference = @(
        { param($n) $n -eq     'qwen2.5-coder:7b' },
        { param($n) $n -like   'qwen2.5-coder*'    },
        { param($n) $n -like   'qwen3-coder*'      },
        { param($n) $n -match  '(?i)coder'         }
    )
    foreach ($test in $preference) {
        foreach ($n in $names) {
            if (& $test $n) { $primaryModel = $n; break }
        }
        if ($primaryModel) { break }
    }

    if ($primaryModel) { Pass ("Coder model present: {0}" -f $primaryModel) }
    else { Fail "No coder model found (looked for qwen2.5-coder* or qwen3-coder*). Run INSTALL-EVERYTHING.bat." }
} else {
