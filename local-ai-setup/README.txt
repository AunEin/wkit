============================================================
  CP2077 Local AI Setup v4.3  --  EASY MODE EDITION
  (one-click install, friendly visual UI, no commands,
   pre-loaded with pose-modding knowledge, model pulls that
   actually work, no UAC self-elevation, never closes silently)
============================================================

KNOWN BUG FIXED IN v4.3:
  v4.2 tried to run 'ollama pull qwen3-coder:7b' and
  'ollama pull qwen3-coder:30b-a3b'. NEITHER tag exists on
  ollama.com/library -- qwen3-coder has no 7B variant at all,
  and the 30B-A3B tag needs an explicit quantization suffix.
  So both pulls failed with "pull model manifest: file does
  not exist" and users ended up with no coder model, and the
  smoke test then "failed" looking for qwen3-coder.
  v4.3 pulls tags that actually exist:
    - qwen2.5-coder:7b           (baseline, ~4.7 GB)
    - qwen3-coder:30b-a3b-q4_K_M (optional big, ~19 GB)
  and VERIFY-INSTALL.bat accepts either coder family.
...
KNOWN BUG FIXED IN v4.2:
  ...
  with nothing happening. v4.2+ does NOT auto-elevate and
...
----------------------------------
  ...
  1. Double-click   INSTALL-EVERYTHING.bat
     ...
     The script then downloads + installs everything:
       ...
       - Qwen2.5-Coder 7B (the AI brain)        ~4.7 GB
...
(qwen3-coder:30b-a3b-q4_K_M, 19 GB). It asks Y/N before
downloading since it's a large file. You can re-run the
installer ANY TIME to add it later -- already-installed
pieces are skipped.
